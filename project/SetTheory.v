(** This section contains theorems and definitions of set theory, it includes
...
(* Un ensemble est une fonction d'un type quelconque U vers Prop
    i.e. {Vrai,Faux}. Ainsi, pour tout ensemble S, c'est à dire une fonction
    d'un type U (donné) vers {Vrai,Faux}, S est l'ensemble vide (du type U) ssi
    pour tout élément x du type U, on a que non S(x) i.e. x \notin S
    Le théorème is_empty dépend du type U *)

(* Equivalent definition of an empty set *)
Theorem is_empty {U : Type} : forall S, 
  (forall x, ~ S x) <-> S = Empty_set U.
...
Theorem non_empty {U : Type} : forall S,
  (exists n,  S n) <-> S <> Empty_set U.
...
(* Definition of the existence of the minimum / function that returns the set of minimums *)
...
(* Forall sets A B such that A \subseteq B, the minimum of A is \leq than the minimum of B*)
...
Axiom wop : forall (N: Ensemble nat), 
  N <> Empty_set nat -> set_ex_minimum N le le_order.

(* Minimum of a natural set *)
Definition nat_min (N: Ensemble nat) (ne: N <> Empty_set nat) := 
  set_minimum N le le_order (wop N ne).

(* Definition of a set of natural numbers bounded by above*)
Definition nat_bounded (N: Ensemble nat): Prop := 
  set_bounded_above N le le_order.

(* A bounded non empty set has always a maximum *)
Theorem ex_nat_max : forall N,
  nat_bounded N -> N <> Empty_set nat -> set_ex_maximum N le le_order.
...
Definition nat_max (N: Ensemble nat) (b: nat_bounded N) (ne: N <> Empty_set nat) : nat :=
  set_maximum N le le_order (ex_nat_max N b ne).

(* N is not empty *)
Theorem n_non_empty : 
  universe <> Empty_set nat.
...
Definition R_bounded (S: Ensemble R): Prop := 
  set_bounded_above S Rle Rle_order.
...
Definition R_max (S: Ensemble R) (b: R_bounded S) (ne: S <> Empty_set R) : R := 
  set_maximum S Rle Rle_order (ex_R_max S b ne).**)


(** ** Intervals *)

(* ]a, b] u {a} = [a, b] *)
Lemma set_f_t_plus_f : forall from to : R, 
  (from <= to)%R -> Add R (fun x: R => (x > from)%R /\ (x <= to)%R) from = (fun x => (x >= from)%R /\ (x <= to)%R).
...
Proof.  
  ...
  (* [a, b[ u {b} = [a, b] *)
Lemma set_f_t_plus_t : forall from to :R, 
  (from <= to)%R -> Add R (fun x: R => (x >= from)%R /\ (x < to)%R) to = (fun x => (x >= from)%R /\ (x <= to)%R).
...
Definition set_opp (S: Ensemble R): Ensemble R := 
  (fun x => S (-x)%R).

(* The opposite of the opposite of a set is the original set *)
Theorem set_opp_involutive: forall S, 
  set_opp (set_opp S) = S.
...
Theorem set_opp_non_empty: forall S, 
  S <> Empty_set R <-> set_opp S <> Empty_set R.
...
Definition set_opp_Rbar (S: Ensemble Rbar): Ensemble Rbar := 
  (fun x => S (Rbar_opp x)).

(* The opposite of the opposite of a set is the original set *)
Theorem set_opp_Rbar_involutive: forall S, 
  set_opp_Rbar (set_opp_Rbar S) = S.
...
(* If a set is non empty then its opposite will neither be empty *)
Theorem set_opp_Rbar_non_empty: forall S, 
  S <> Empty_set Rbar <-> set_opp_Rbar S <> Empty_set Rbar.
...
Definition set_nat_to_r (N: Ensemble nat) := 
  fun r => exists n, N n /\ r = INR n.
...
Theorem set_opp_R_Rbar : forall S: Ensemble R, 
  set_opp_Rbar (set_R_to_Rbar S) = set_R_to_Rbar (set_opp S).
...
Theorem set_R_to_Rbar_empty : forall S, 
  S = Empty_set R <-> set_R_to_Rbar S = Empty_set Rbar.
